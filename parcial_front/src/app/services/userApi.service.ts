...
import { Permiso, Permisos } from '../models/permisos.interface';
...
export class UserApiService {
  ...
  public login(user: User) {
    this.http.post<DataAuth>(`${this.apiUrl}auth/login/`, user)
      .subscribe((resp) => {
  ...
  constructor() {
    effect(() => {
      ...
      if (!token) {
        // Si no hay token, redirige y limpia el localStorage
        localStorage.removeItem('token');
        localStorage.removeItem('role');
        this.router.navigate(['login']);
      } else {
        // Si hay token, lo actualiza en localStorage
